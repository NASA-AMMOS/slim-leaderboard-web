// Static version of script.js for GitHub Pages
// This version shows instructions instead of making API calls
...
class SlimLeaderboardApp {
    ...
    async handleFormSubmit(event) {
        ...
        try {
            ...
            this.hideProgress();
            this.showError(error.message || 'An error occurred while generating instructions');
        } finally {
    ...
    async generateInstructions(options) {
        ...
        // Generate instructions for running analysis
        const tokenDisplay = githubToken ? `${githubToken.substring(0, 8)}...` : 'your_token_here';
        const analysisNote = `# SLIM Best Practices Analysis Instructions
...
This is the static GitHub Pages version. To run the actual analysis, please use one of the following options:
...
2. **Set your GitHub token:**
   \`\`\`bash
   export GITHUB_TOKEN="${tokenDisplay}"
   \`\`\`
...
### Option 2: Use the Full Web Application

For real-time web analysis with actual results:
...
4. **Open your browser:**
   Visit http://localhost:8081 for the full interactive experience with real-time analysis.

### Option 3: Use Docker

\`\`\`bash
export GITHUB_TOKEN="${tokenDisplay}"
docker run -p 8081:8081 -e GITHUB_TOKEN ghcr.io/nasa-ammos/slim-leaderboard-web
\`\`\`
...
**Learn more:** [nasa-ammos.github.io/slim](https://nasa-ammos.github.io/slim/)
    ...
    showProgress(targetType) {
        ...
        const message = isOrg 
            ? 'Generating organization analysis instructions...' 
            : 'Generating repository analysis instructions...';
